...
from src.profile_context import load_profile_context, reset_profile_context, save_profile_context
...
HOME_FEATURE_IMAGES = {
    "add_job": ROOT_DIR / "assets" / "add_job_boardwalk_feature.png",
    ...
    if st.button("Run Analysis", type="primary"):
        if not description.strip():
            ...
            with st.spinner("Parsing job description, scoring fit, and generating resume emphasis..."):
                ...
                fit, fit_meta = score_role_fit(description, parsed, profile_context=profile_context)
...
def render_dashboard_tab() -> None:
    ...
    if not tracker.empty:
        strong_fit = int(
            (
                tracker["fit_category"].isin(["Primary Target", "Strong Secondary", "Strong Fit"])
                | pd.to_numeric(tracker["fit_score"], errors="coerce").fillna(0).ge(7)
            ).sum()
        )
    ...
    if not tracker.empty:
        ...
        with st.expander("Edit application from dashboard", expanded=False):
            ...
            current_fit_category = selected_job.get("fit_category") or FIT_CATEGORY_OPTIONS[0]
            current_priority = selected_job.get("priority") or PRIORITY_OPTIONS[1]
            current_fit_score = "" if pretty_value(selected_job.get("fit_score")) == "-" else pretty_value(selected_job.get("fit_score"))
            ...
            with st.form(f"dashboard_edit_form_{selected_job_id}"):
                ...
                with left:
                    ...
                    fit_score = st.text_input(
                        "Fit Score (1-10)",
                        value=current_fit_score,
                        placeholder="8.5",
                    )
                    fit_category = st.selectbox(
                        "Fit Category",
                        FIT_CATEGORY_OPTIONS,
                        index=FIT_CATEGORY_OPTIONS.index(current_fit_category) if current_fit_category in FIT_CATEGORY_OPTIONS else 0,
                        key=f"dashboard_fit_category_{selected_job_id}",
                    )
                with right:
                    ...
                    priority_override = st.selectbox(
                        ...
                        key=f"dashboard_priority_{selected_job_id}",
                    )
            ...
            if save_edit:
                if not company.strip() or not job_title.strip():
                    ...
                    if application_date and not application_iso:
                        ...
                        if fit_score.strip():
                            try:
                                fit_score_value = max(1, min(10, round(float(fit_score), 1)))
                            except ValueError:
                                st.error("Fit score must be a number from 1 to 10.")
                                return
                        merged_notes = upsert_labeled_line(notes, "JD link", jd_link)
                        merged_notes = upsert_labeled_line(merged_notes, "Company site", company_site)
                        priority = compute_priority(
                            ...
                            outreach_status=outreach_status,
                            status=application_status,
                            manual_priority=priority_override,
                        )
                        update_job_fields(
                            selected_job_id,
                            {
                                ...
                                "outreach_status": outreach_status,
                                "fit_score": fit_score_value,
                                "fit_category": None if fit_category == "Not Scored" else fit_category,
                                "job_description": job_description.strip(),
                                "notes": merged_notes,
                                "priority": priority,
                            },
...
def render_outreach_tab() -> None:
    ...
    custom_context = st.text_area(
        ...
        placeholder="Example: Reached out after a webinar, met through an alumni group, or found a relevant team lead.",
        height=100,
    )
...
def render_settings_tab() -> None:
    render_section_header(
        "Profile Settings",
        "Update the evaluation context used by the LLM for role scoring, resume angles, and recommendations.",
        kicker="Guidance Console",
    )
    st.caption("Edit the evaluation context used for fit scoring, resume angles, and recommendations. Local custom context is stored outside the public repo.")
    current_context = load_profile_context()
    with st.form("profile_context_form"):
        updated_context = st.text_area(
            "LLM evaluation context",
            value=current_context,
            height=520,
            help="Describe target roles, skills, constraints, values, locations, red flags, and scoring preferences.",
        )
        left, right = st.columns(2)
        with left:
            save_context = st.form_submit_button("Save Context", type="primary", use_container_width=True)
        with right:
            reset_context = st.form_submit_button("Reset To Public Default", use_container_width=True)

    if save_context:
        if not updated_context.strip():
            st.error("Context cannot be empty.")
        else:
            save_profile_context(updated_context)
            st.success("LLM context updated.")
            st.rerun()

    if reset_context:
        reset_profile_context()
        st.success("Restored the public default evaluation context.")
        st.rerun()
...
def main() -> None:
    ...
    home_tab, add_tab, analyze_tab, dashboard_tab, outreach_tab, settings_tab = st.tabs(
        [
            "Home",
            "Add Job",
            "Analyze JD",
