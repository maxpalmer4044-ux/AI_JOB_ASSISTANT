...
Streamlit Cloud does not read a local `.env` file. Add these values in the Streamlit app settings under `Secrets`:
...
The file `.streamlit/secrets.example.toml` shows the same format. Do not rename it to `secrets.toml` for a public commit.

Do not commit `.env`, `data/jobs.db`, or `data/profile_context.md` to GitHub.
...
By default, the app uses the public starter profile context in `src/profile_context.py`. When a user saves a custom context, it is stored locally at:
